...
The rig stage's built-in `auto-rig-bbox` adapter classifies the reconstructed mesh as humanoid or quadruped (core-axis ratio plus a belly-gap check that keeps lying-down humanoid reconstructions out of the quadruped path) and reports the result as the `bodyType` artifact metric. Humanoids get the 14-joint bbox-proportion skeleton; quadrupeds get a measured horizontal-spine layout (front legs bound to the Arm chain, rear legs to the Leg chain, optional Tail) so SMPL human walk retargeting produces a diagonal trot, plus a synthesized tail-sway channel at bake time. Orientation is normalized before joint placement: side-facing meshes (length along X) are yaw-rotated 90° and backward-facing meshes are flipped 180° using above-spine vertex-mass head detection. Both rig and motion stages are gated by `tools/matrix/validate_character.py` (`renderValid` metric), whose upright check branches on the skin marker (`AutoQuadrupedRig`/`AutoHumanoidRig`).
