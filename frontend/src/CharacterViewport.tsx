...
import {buildMotionClip, detectBoneMap} from './motion/motionClip';
import type {BoneMap} from './motion/motionClip';
...
export default function CharacterViewport({playing,clip,speed,time,showSkeleton,modelUrl='/models/Soldier.glb',motion=null,onState}:Props){
 ...
 useEffect(()=>{
  ...
  scene.add(new THREE.HemisphereLight(0xffffff,0xdfe6f0,2.3)); const key=new THREE.DirectionalLight(0xfff4e8,3.2); key.position.set(3,6,4); key.castShadow=true; key.shadow.mapSize.set(2048,2048); scene.add(key); scene.add(key.target); const rim=new THREE.DirectionalLight(0xbcd0ff,1.6);rim.position.set(-4,3,-4);scene.add(rim);
  ...
  let hipsNode:THREE.Object3D|undefined; const followPos=new THREE.Vector3();
  ...
  new GLTFLoader().load(modelUrl,gltf=>{if(disposed)return;model=gltf.scene; const box=new THREE.Box3().setFromObject(model);const size=box.getSize(new THREE.Vector3());const center=box.getCenter(new THREE.Vector3());const scale=2.75/Math.max(size.y,.01);model.scale.setScalar(scale);model.position.set(-center.x*scale,-box.min.y*scale,-center.z*scale);const boneNames:string[]=[]; model.traverse(o=>{if((o as THREE.Mesh).isMesh){const m=o as THREE.Mesh;m.castShadow=true;m.receiveShadow=true} if((o as THREE.Bone).isBone)boneNames.push(o.name)}); boneMap=detectBoneMap(boneNames); hipsNode=boneMap.hips?model.getObjectByName(boneMap.hips)??undefined:undefined; const actor=new THREE.Group();actor.name='actor-root';actor.add(model);scene.add(actor);clips=[...gltf.animations];mixer=new THREE.AnimationMixer(actor);helper=new THREE.SkeletonHelper(model);helper.visible=stateRef.current.showSkeleton;const skeletonMaterial=helper.material as THREE.LineBasicMaterial;skeletonMaterial.depthTest=false;skeletonMaterial.transparent=true;skeletonMaterial.opacity=.8;scene.add(helper);report();},undefined,e=>report(`Model load failed: ${String(e)}`));
  ...
  const m=s.motion??null;if(mixer&&m!==lastMotion){lastMotion=m;if(directed){mixer.uncacheClip(directed);clips=clips.filter(c=>c!==directed);if(active===directed.name){action?.stop();action=undefined;active='';duration=0}}directed=m?buildMotionClip(m,boneMap):undefined;if(directed)clips=[...clips,directed];report()}
  ...
  // 캐릭터가 화면을 벗어나지 않게 힙스 XZ를 지수 감쇠로 추적 (y는 고정해 상하 흔들림 방지)
  if(hipsNode){hipsNode.getWorldPosition(followPos);const k=1-Math.exp(-6*timer.getDelta());const fx=(followPos.x-controls.target.x)*k,fz=(followPos.z-controls.target.z)*k;controls.target.x+=fx;controls.target.z+=fz;camera.position.x+=fx;camera.position.z+=fz;
   // 바닥·그리드·키라이트도 타깃을 따라 이동 — 장거리 클립(sprint ~11m)에서 바닥 이탈·그림자 소실 방지 (그리드는 셀 0.5m 스냅으로 월드 고정처럼 보이게)
   const tx=controls.target.x,tz=controls.target.z;floor.position.x=tx;floor.position.z=tz;grid.position.x=Math.round(tx/.5)*.5;grid.position.z=Math.round(tz/.5)*.5;key.position.set(tx+3,6,tz+4);key.target.position.set(tx,0,tz)}
  renderer.render(scene,camera)};draw(performance.now());
